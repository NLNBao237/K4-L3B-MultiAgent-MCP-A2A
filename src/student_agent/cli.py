    ...
    while pending:
        try:
            ...
            if failures > MAX_RECONNECTS:
                raise RuntimeError(f"MCP connection keeps failing: {exc!r}") from exc
            print(f"WARN: connection lost at {pending[0]} ({exc!r}); reconnecting", flush=True)
...
def _is_fatal(exc: BaseException) -> bool:
    """Contract/configuration errors abort the run; transport failures are retried."""
    if isinstance(exc, BaseExceptionGroup):
        return any(_is_fatal(inner) for inner in exc.exceptions)
    return isinstance(exc, ValueError) or (
        isinstance(exc, RuntimeError) and "no tools" in str(exc)
    )
